...
export const coreWordsChunk: ChunkDefinition = {
  ...
  required: true,
  loader: async () => {
    const { koToEnWords } = await import('../../dictionary/entries/words');
    return koToEnWords;
  },
};
...
export const idiomsChunk: ChunkDefinition = {
  ...
  required: false,
  loader: async () => {
    const { idioms } = await import('../../dictionary/entries/idioms');
    const result: Record<string, string> = {};
...
export const onomatopoeiaChunk: ChunkDefinition = {
  ...
  required: false,
  loader: async () => {
    const { onomatopoeia } = await import('../../dictionary/entries/onomatopoeia');
...
export const culturalChunk: ChunkDefinition = {
  ...
  required: false,
  loader: async () => {
    const { culturalExpressions } = await import('../../dictionary/entries/cultural');
    // 이미 Record<string, string> 형태이므로 바로 반환
