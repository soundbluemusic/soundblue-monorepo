...
```typescript
// vite.config.ts - TanStack Start SSR 설정
...
// ❌ SPA 모드 (금지!)
// TanStack Start는 기본적으로 SSR이므로
// SSR을 비활성화하면 안 됨
```

### 검증 체크리스트

- [ ] `vite.config.ts`에 `tanstackStart()` 플러그인 확인
- [ ] SSR 모드로 동작 (TanStack Start 기본값)
- [ ] Cloudflare Workers 배포 설정 확인
...
이 스킬은:
1. `vite.config.ts`에서 TanStack Start 플러그인 확인
2. SSR 설정 검증
...
║                                                                              ║
║  📋 새 앱/라우트 생성 시:                                                      ║
║  ├── TanStack Start + SSR 기본 설정 사용                                      ║
║  └── Cloudflare Workers 배포 설정 포함                                        ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
```
