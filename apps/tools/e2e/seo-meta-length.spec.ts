...
/**
 * Tools App - SEO Meta Length Optimization Tests
 *
 * Google 검색 결과 표시 기준:
...
test.describe('SEO Meta Length Optimization', () => {
  const languages = ['', '/ko'];
  const routes = ['/', '/tools', '/privacy', '/terms'];
  ...
  test.describe('제목(Title) 길이 검증', () => {
    for (const lang of languages) {
      for (const route of routes) {
        test(`${lang}${route} - 제목 20-70자`, async ({ page }) => {
          await page.goto(`${lang}${route}`);
    ...
    for (const lang of languages) {
      for (const route of routes) {
        test(`${lang}${route} - 브랜드명 포함`, async ({ page }) => {
          ...
          // 브랜드명이나 앱 이름 포함 확인
          expect(title).toMatch(/Tools|도구|SoundBlue/i);
        });
      }
    }
  });
  ...
  test.describe('도구 페이지 메타 길이', () => {
    const tools = ['/translator', '/metronome', '/qr-generator', '/color-harmony'];

    for (const tool of tools) {
      test(`${tool} - 도구 페이지 메타 최적화`, async ({ page }) => {
