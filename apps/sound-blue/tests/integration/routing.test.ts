...
describe('React Router Configuration', () => {
  ...
  describe('다국어 라우트 일관성', () => {
    it('모든 영어 라우트에 대응하는 한국어 라우트 존재', async () => {
      ...
      const enRoutes = routes.filter(
        (route) => !route.startsWith('/ko') && route !== '/offline',
      );
      const koRoutes = routes.filter((route) => route.startsWith('/ko'));

      const missingKoRoutes: string[] = [];

      enRoutes.forEach((enRoute) => {
      ...
      expect(
        missingKoRoutes,
        `Missing Korean routes: ${missingKoRoutes.join(', ')}`,
      ).toEqual([]);
    });
    ...
    it('영어와 한국어 라우트 수가 동일 (offline 제외)', async () => {
      ...
      expect(
        enRoutes.length,
        `English: ${enRoutes.length}, Korean: ${koRoutes.length}`,
      ).toBe(koRoutes.length);
    });
  });
  ...
  describe('라우트 형식 검증', () => {
    it('모든 라우트가 슬래시(/)로 시작', async () => {
      ...
      const invalidRoutes = routes.filter((route) => !route.startsWith('/'));
    ...
    it('라우트에 공백 없음', async () => {
      ...
      const routesWithSpaces = routes.filter((route) => route.includes(' '));
    ...
    it('라우트가 슬래시(/)로 끝나지 않음 (루트 제외)', async () => {
      ...
      const routesEndingWithSlash = routes.filter(
        (route) => route !== '/' && route !== '/ko' && route.endsWith('/'),
      );
  ...
  describe('Edge Cases', () => {
    ...
    it('undefined 또는 null 라우트 없음', async () => {
      ...
      const invalidRoutes = routes.filter((route) => route === undefined || route === null);
  ...
  describe('라우트 명명 규칙', () => {
    it('라우트는 소문자 사용 (케밥 케이스)', async () => {
      ...
      const uppercaseRoutes = routes.filter((route) => /[A-Z]/.test(route));
    ...
    it('라우트는 하이픈(-) 구분자 사용', async () => {
      ...
      const invalidSeparators = routes.filter((route) => route.includes('_'));

      expect(
        invalidSeparators,
        `Routes with underscore: ${invalidSeparators.join(', ')}`,
      ).toEqual([]);
    });
  });
});
