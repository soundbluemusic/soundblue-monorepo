import { createFileRoute } from '@tanstack/react-router';
import { NavigationLayout } from '~/components/layout';
import m from '~/lib/messages';

export const Route = createFileRoute('/ko/terms')({
  head: () => ({
    meta: [
      { title: '이용약관 | Sound Blue' },
      {
        name: 'description',
        content: 'Sound Blue 웹사이트 및 서비스 이용약관입니다.',
      },
    ],
    links: [
      { rel: 'canonical', href: 'https://soundbluemusic.com/ko/terms' },
      { rel: 'alternate', hrefLang: 'ko', href: 'https://soundbluemusic.com/ko/terms' },
      { rel: 'alternate', hrefLang: 'en', href: 'https://soundbluemusic.com/terms' },
      { rel: 'alternate', hrefLang: 'x-default', href: 'https://soundbluemusic.com/terms' },
    ],
  }),
  component: Terms,
});

function Terms() {
  return (
    <NavigationLayout>
      <div className="max-w-3xl mx-auto p-6 prose">
        <h1>{m['terms.title']()}</h1>
        <h2>{m['terms.sections.use.title']()}</h2>
        <p>{m['terms.sections.use.content']()}</p>
        <h2>{m['terms.sections.copyright.title']()}</h2>
        <p>{m['terms.sections.copyright.content']()}</p>
        <h2>{m['terms.sections.disclaimer.title']()}</h2>
        <p>{m['terms.sections.disclaimer.content']()}</p>
        <h2>{m['terms.sections.changes.title']()}</h2>
        <p>{m['terms.sections.changes.content']()}</p>
      </div>
    </NavigationLayout>
  );
}
