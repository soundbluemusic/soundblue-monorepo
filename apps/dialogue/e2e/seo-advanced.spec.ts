...
/**
 * Dialogue App - Advanced SEO Tests
 *
 * 검증 항목:
  ...
  test('WebSite 또는 SoftwareApplication 타입', async ({ page }) => {
    ...
    for (const script of jsonLdScripts) {
      ...
      if (
        ...
        jsonLd['@type'] === 'EducationalApplication'
      ) {
        hasValidType = true;
    ...
    expect(hasValidType, 'Must have WebSite or Application type').toBe(true);
  });
});
...
test.describe('Advanced SEO - Canonical URL', () => {
  ...
  test('Canonical URL에 도메인 포함', async ({ page }) => {
    ...
    expect(canonical, 'Canonical must include domain').toContain('dialogue.soundbluemusic.com');
  });

  test('언어별 Canonical URL 정확성', async ({ page }) => {
    await page.goto('/');
    const enCanonical = await page.getAttribute('link[rel="canonical"]', 'href');
    expect(enCanonical).toMatch(/dialogue\.soundbluemusic\.com\/?$/);

    await page.goto('/ko');
    const koCanonical = await page.getAttribute('link[rel="canonical"]', 'href');
    expect(koCanonical).toMatch(/dialogue\.soundbluemusic\.com\/ko\/?$/);
  });
