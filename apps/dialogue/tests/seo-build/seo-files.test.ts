...
describe('SEO Files', () => {
  ...
  describe('sitemap.xml', () => {
    ...
    it('도메인 포함', () => {
      const sitemap = readFile(`${BUILD_DIR}/sitemap.xml`);
      expect(sitemap).toContain('dialogue.soundbluemusic.com');
    });
  });

  describe('페이지별 sitemap 검증', () => {
  ...
  describe('PWA 파일', () => {
    it('manifest.json 파일 존재', () => {
      const manifestExists =
        fileExists(`${BUILD_DIR}/manifest.json`) || fileExists(`${BUILD_DIR}/manifest.webmanifest`);
      expect(manifestExists).toBe(true);
    });

    it('favicon 존재', () => {
      const faviconExists =
        fileExists(`${BUILD_DIR}/favicon.ico`) || fileExists(`${BUILD_DIR}/favicon.png`);
